  ...
  */
  public static void Create(int difficulty)
  {
    ...
    for ( int i = 0; i < nbrservavecAntivirus ; i++ ) {
      ...
      int serveuraleatoire = Rng.getRandomInt(1, tab.length); //selectionne un serveur dans le tableau, excluant le serveur0
    ...
    for (int i = 0; i < nbrServAvecBitcoin ; i++ ) {
      int numtmp = Rng.getRandomInt(1, tab.length); //selectionne un serveur aleatoirement, en dehors du serveur0
    ...
    for (int i = 1 ; i < tab.length ; i++ ) {
      ...
      for ( int j = 0 ; j < nbr ; j++ ) {
        int serv = Rng.getRandomInt(0, tab.length);//selectionne un serveur au hasard
    ...
    int nombredeservAvecmdp = (int)(Rng.getRandomInt(10, tab.length) * multiplier3);  //retourne un nombre aleatoire de serveurs entre 10 et la longueur du tableau
    ...
    for (int i = 0; i < nombredeservAvecmdp ; i++ ) {
      int servAleatoire = Rng.getRandomInt(1, tab.length); //selectionne un nombre entre 1 (serveur0 ne doit pas avoir de mdp) et la longueur du tableau
